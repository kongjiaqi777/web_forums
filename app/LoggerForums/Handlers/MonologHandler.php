...
use App\LoggerForums\Exceptions\LogUnexpectedValueException;
use App\LoggerForums\Exceptions\LogInvalidArgumentException;
use App\LoggerForums\Extend\FluentdHandler;
use App\LoggerForums\Extend\LineFormatter;
use App\LoggerForums\Extend\FluentdFormatter;
...
class MonologHandler extends Handler implements PsrLoggerInterface
{
    ...
    {
        if (strlen($level) === 0) {
            throw new LogInvalidArgumentException("Level is empty");
        }
        $this->level = $level;
    }
    ...
    public function useFluentd($path, $level = 'debug')
    {
        ...
        $this->handler->pushHandler($handler = new FluentdHandler($logger, $level));
    ...
    {
        $this->handler->pushHandler($handler = new StreamHandler($path, $level));
     ...
     * @return void
     */
    public function useDailyFiles($path, $level = 'debug', $days = 0)
    {
     ...
     * @return void
     */
    public function useHourlyFiles($path, $level = 'debug', $hours = 1)
    {
     ...
     * @return \App\LoggerForums\Extend\FluentdFormatter
     */
