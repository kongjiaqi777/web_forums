...
abstract class Processor
{
    ...
    public function getDefaultFormat()
    {
        if (is_null($this->defaultFormat)) {
            $this->defaultFormat = config('forumslogger.default_format');
        }
    ...
    protected function getOperatorType()
    {
        ...
        $this->operatorType = $allParams['operator_type'] ?? 0;
    ...
    {
        $allParams = app('request')->all();
        $this->operatorId = $allParams['operator_id'] ?? 0;
    ...
    protected function getFrom()
    {
        ...
        $allParams = app('request')->all();
        return Arr::get($allParams, 'from', 0);
    }
    ...
    protected function getEvent($type)
    {
        ...
        if (!empty($type)) {
            return Arr::get($this->events, $type);
        }
