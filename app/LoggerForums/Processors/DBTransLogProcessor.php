...
class DBTransLogProcessor extends SQLLogProcessor
{
    ...
    {
        $enabled = config('forumslogger.trans_log.enabled', false);
        if (!$enabled) {
            return;
        }
    ...
    public function getPath()
    {
        ...
        $this->path = config('forumslogger.trans_log.path');
        return $this->path;
    }
    ...
    public function getType()
    {
        ...
        $this->type = config('forumslogger.trans_log.type');
        return $this->type;
    }
    ...
    public function getLevel()
    {
        ...
        $this->level = config('forumslogger.trans_log.level', 'debug');
        return $this->level;
    }
    ...
    public function getFormat()
    {
        if (is_null($this->format)) {
            $this->format = config('forumslogger.trans_log.format', 'DB TRANSACTION: {sql}');
        }
        return $this->format;
    }
    ...
    protected function initListen()
    {
        ...
        try {
            ...
            throw new \Exception('forumslogger DB listen failed: ' . $e->getMessage(), $e->getCode());
        }
    }
