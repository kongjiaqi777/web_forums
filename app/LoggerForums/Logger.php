...
namespace App\LoggerForums;

use App\LoggerForums\Processors\BaseLogProcessor;
use App\LoggerForums\Processors\RequestLogProcessor;
use App\LoggerForums\Processors\ExceptionLogProcessor;
use App\LoggerForums\Processors\DBTransLogProcessor;
use App\LoggerForums\Processors\SQLLogProcessor;
use App\LoggerForums\Exceptions\LogBadMethodCallException;
...
class Logger
{
    ...
    protected function checkConsole()
    {
        ...
        // 命令行下默认同步输出日志
        config(['forumslogger.base_log.async' => false]);
        ...
        if (strpos($command, 'phpunit') != false) {
            config([
                'forumslogger.request_log.enabled' => false,
                'forumslogger.exception_log.enabled' => false,
            ]);
    ...
    {
        $warningTag = config('forumslogger.warning_tag.business', '业务报警');
    ...
    public function custom($channel = 'base', $path = '')
    {
        ...
        if (!config('forumslogger.base_log.enabled', false)) {
            return $this->handler;
        }
